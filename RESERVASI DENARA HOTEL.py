import streamlit as stimport pandas as pdfrom datetime import datetime, date
...
FASILITAS_KAMAR = {"Standard Room": ["Free Wi-Fi", "Air Conditioning", "Smart TV 32"", "Shower Bathroom", "Complimentary Water"],"Superior Room": ["Free Wi-Fi", "Air Conditioning", "Smart TV 43"", "Water Heater", "Mini Refrigerator", "Coffee & Tea Maker"],"Deluxe Room": ["Free Wi-Fi", "Air Conditioning", "Smart TV 55"", "Luxury Bathtub", "Mini Bar", "Safety Deposit Box", "Private Balcony"],"Suite Room": ["Free Wi-Fi", "Air Conditioning", "Smart TV 65"", "Private Jacuzzi", "Premium Mini Bar", "Separate Living Room", "24/7 Butler Service", "Private Swimming Pool"]}

Menu makanan buat room service beserta harganya yang ramah kantong

MENU_MAKANAN = {"Nasi Goreng Kampung": 35000,"Ayam Goreng Sambal Matah": 30000,"Ayam Goreng Serundeng": 40000,"Es Teh Manis Premium": 12000,"Kopi Susu Aren": 20000,"Kwetiaw Goreng": 20000,"Indomie Goreng + telur": 19000,"Butterscotch Latte": 22000,"Nasi Ayam Geprek": 23000}

Tempat nyimpen status kamar biar datanya gak ilang pas aplikasinya di-refresh

if "kamar_data" not in st.session_state:st.session_state.kamar_data = [{"No Kamar": "101", "Tipe Kamar": "Standard Room", "Status": "🟩 Tersedia"},{"No Kamar": "102", "Tipe Kamar": "Standard Room", "Status": "🟨 Direservasi"},{"No Kamar": "103", "Tipe Kamar": "Standard Room", "Status": "🟩 Tersedia"},{"No Kamar": "104", "Tipe Kamar": "Standard Room", "Status": "🟩 Tersedia"},{"No Kamar": "105", "Tipe Kamar": "Standard Room", "Status": "🟩 Tersedia"},{"No Kamar": "201", "Tipe Kamar": "Superior Room", "Status": "🟩 Tersedia"},{"No Kamar": "202", "Tipe Kamar": "Superior Room", "Status": "🟨 Direservasi"},{"No Kamar": "203", "Tipe Kamar": "Superior Room", "Status": "🟩 Tersedia"},{"No Kamar": "204", "Tipe Kamar": "Superior Room", "Status": "🟩 Tersedia"},{"No Kamar": "301", "Tipe Kamar": "Deluxe Room", "Status": "🟩 Tersedia"},{"No Kamar": "302", "Tipe Kamar": "Deluxe Room", "Status": "🟨 Direservasi"},{"No Kamar": "303", "Tipe Kamar": "Deluxe Room", "Status": "🟩 Tersedia"},{"No Kamar": "401", "Tipe Kamar": "Suite Room", "Status": "🟩 Tersedia"},{"No Kamar": "402", "Tipe Kamar": "Suite Room", "Status": "🟩 Tersedia"},{"No Kamar": "403", "Tipe Kamar": "Suite Room", "Status": "🟨 Direservasi"},]

Data awal tamu yang ceritanya udah duluan booking biar aplikasi gak keliatan sepi

if "reservasi_log" not in st.session_state:st.session_state.reservasi_log = [{"id": "RSV-102202", "nama": "Budi Santoso", "hp": "+62 8123444", "email": "budi@gmail.com", "kamar": "102", "tipe": "Standard Room", "check_in": "2026-06-01", "check_out": "2026-06-05", "total_biaya": 2600000, "sudah_dibayar": 2600000, "status_bayar": "Lunas (100%)", "metode": "Transfer BCA", "status": "🟨 Direservasi", "food_charge": 0},{"id": "RSV-202202", "nama": "Siti Rahma", "hp": "+62 8125555", "email": "siti@gmail.com", "kamar": "202", "tipe": "Superior Room", "check_in": "2026-06-03", "check_out": "2026-06-07", "total_biaya": 4000000, "sudah_dibayar": 2000000, "status_bayar": "Bayar Setengah (DP 50%)", "metode": "Mandiri Virtual Account", "status": "🟨 Direservasi", "food_charge": 65000},{"id": "RSV-403202", "nama": "Rayyanza", "hp": "+62 8129999", "email": "rayyanza@gmail.com", "kamar": "403", "tipe": "Suite Room", "check_in": "2026-06-05", "check_out": "2026-06-12", "total_biaya": 66500000, "sudah_dibayar": 66500000, "status_bayar": "Lunas (100%)", "metode": "Dana", "status": "🟨 Direservasi", "food_charge": 0},]

Inisialisasi tempat penampungan riwayat transaksi, pembatalan, pesanan makanan, voucher, dan review tamu

if "histori_transaksi" not in st.session_state:st.session_state.histori_transaksi = [{"id": "RSV-101000", "nama": "Joko Widodo", "kamar": "101", "tipe": "Standard Room", "grand_total": 1300000, "status": "✅ Selesai (Check-Out)"}]if "log_pembatalan" not in st.session_state: st.session_state.log_pembatalan = []if "makanan_log" not in st.session_state: st.session_state.makanan_log = []if "kode_voucher_input" not in st.session_state: st.session_state.kode_voucher_input = ""if "voucher_terpasang" not in st.session_state: st.session_state.voucher_terpasang = ""if "ulasan_log" not in st.session_state:st.session_state.ulasan_log = [{"nama": "Andi Pratama", "rating": 5, "komentar": "Keren bgt, nginep di lantai 4 berasa eksklusif!"}]
...
st.sidebar.title("🏨 Denara Hotel")menu_utama = st.sidebar.radio("Menu Utama", ["🏠 Dashboard","🏨 Manajemen Kamar","💳 Area Transaksi Tamu","🍽️ Room Service","⭐ Penilaian Hotel","🛟 Bantuan"])

Logika percabangan buat nampilin sub-menu berdasarkan menu utama yang dipilih

if menu_utama == "🏠 Dashboard": pilihan_menu = "🏠 Dashboard"elif menu_utama == "🏨 Manajemen Kamar":pilihan_menu = st.sidebar.radio("Sub-Menu Kamar", ["📝 Reservasi Baru", "🏨 Katalog Kamar", "🗺️ Denah Kamar"])elif menu_utama == "💳 Area Transaksi Tamu":pilihan_menu = st.sidebar.radio("Sub-Menu Transaksi", ["💳 Pembayaran Reservasi Hotel", "🔍 Cek Detail & Check-Out", "📜 Histori & Pembatalan"])elif menu_utama == "🍽️ Room Service":pilihan_menu = st.sidebar.radio("Sub-Menu Room Service", ["🍽️ Pesan Makanan", "💳 Bayar Room Service"])elif menu_utama == "⭐ Penilaian Hotel": pilihan_menu = "⭐ Ulasan Kepuasan"elif menu_utama == "🛟 Bantuan":pilihan_menu = st.sidebar.radio("Sub-Menu Bantuan", ["❓ Pusat Bantuan", "📞 Kontak Layanan Service"])
...
elif pilihan_menu == "📝 Reservasi Baru":st.title("📝 Registrasi Menginap (Reservasi Baru)")col_kiri, col_kanan = st.columns([1.5, 1])
...
elif pilihan_menu == "🏨 Katalog Kamar":st.title("🏨 Katalog Pilihan & Spesifikasi Eksklusif Kamar")st.write("Temukan kenyamanan terbaik selama menginap di Denara Hotel:")
...
elif pilihan_menu == "🗺️ Denah Kamar":st.title("🗺️ Map Letak Kamar Hotel Lantai 1 s/d 4")
...
elif pilihan_menu == "💳 Pembayaran Reservasi Hotel":st.title("💳 Menu Pembayaran Billing Kamar")# Validasi biar gak ada tamu ilegal yang masuk menu ini tanpa ngisi form reservasi duluif "proses_checkout" not in st.session_state:st.warning("Belum ada data pembayaran. Silakan lakukan reservasi kamar terlebih dahulu.")st.stop()
...
elif pilihan_menu == "🔍 Cek Detail & Check-Out":st.title("🔍 Menu Cek Data & Check-Out Mandiri (Akses Tamu)")st.write("Silakan masukkan salah satu data identitas booking Anda untuk memuat rincian billing kamar.")
...
elif pilihan_menu == "📜 Histori & Pembatalan":st.title("📜 Menu Riwayat & Pembatalan Mandiri Tamu")tab_riwayat, tab_batal = st.tabs(["Arsip Histori Menginap", "Ajukan Pembatalan Kamar"])
...
elif pilihan_menu == "🍽️ Pesan Makanan":st.title("🍽️ Room Service Kuliner - Kirim Ke Kamar")st.subheader("Verifikasi Hunian Kamar")input_verifikasi = st.text_input("Konfirmasi Nomor Kamar / Nama Tamu / ID Booking Anda Saat Ini:")
...
elif pilihan_menu == "💳 Bayar Room Service":st.title("💳 Kasir Tagihan Room Service Kuliner Mandiri")input_kasir = st.text_input("Input Nomor Kamar / Nama Tamu / ID Booking Kamu untuk Mengecek Bill Makanan:")if not input_kasir:st.info("Masukkan identitas tamu atau nomor kamar untuk melihat tagihan makanan.")st.stop()
...
if not order:
    ...
    with st.container():
        ...
        if pilihan_metode != "Room Charge (Masuk Bill Kamar Utama)":
            if "Virtual Account" in pilihan_metode:
...
elif pilihan_menu == "⭐ Ulasan Kepuasan":st.title("⭐ Kotak Kepuasan & Review Tamu")st.write("Bagikan pengalaman menginapmu! Feedback kamu sangat berharga bagi kami.")
...
elif pilihan_menu == "❓ Pusat Bantuan":st.title("🛟 FAQ - Pusat Bantuan Informasi")# Pertanyaan populer seputar kebijakan hotel yang dikemas rapi pakai fitur expander Streamlitwith st.expander("⏱️ Jam Berapa Batas Waktu Check-In & Check-Out Standard?"):st.write("Masuk kamar jam 14:00 WIB yaa, kalau keluar maksimal jam 12:00 WIB.")with st.expander("💳 Bisa Bayar Pake QRIS Atau E-Wallet Gak?"):st.write("Bisa bgt! Kita nerima Dana, Gopay, Ovo, ShopeePay, sama Transfer VA Bank.")

--- 12. KONTAK LAYANAN SERVICE ---

elif pilihan_menu == "📞 Kontak Layanan Service":st.title("📞 Kontak Layanan Hotel")
